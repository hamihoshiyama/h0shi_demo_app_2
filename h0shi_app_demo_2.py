...
def load_existing_models():
    if os.path.exists('existing_models.json'):
        with open('existing_models.json', 'r') as f:
            st.session_state.existing_models = json.load(f)
    else:
        st.session_state.existing_models = {}

def save_existing_models():
    with open('existing_models.json', 'w') as f:
        json.dump(st.session_state.existing_models, f)
load_existing_models()

def get_existing_models(username):
    return st.session_state.existing_models.get(username, [])

def save_existing_models():
    with open('existing_models.json', 'w') as f:
        json.dump(st.session_state.existing_models, f)

def get_existing_model(username):
    return st.session_state.existing_models.get(username)
        ...
        if st.session_state.fine_tuning_started and not st.session_state.all_succeeded:
            ...
            while True:
                ...
                if all_succeeded:
                    ...
                    for user_name, model_name in zip(st.session_state.user_names, model_names):
                        st.session_state.existing_models[user_name] = model_name
                    save_existing_models()
